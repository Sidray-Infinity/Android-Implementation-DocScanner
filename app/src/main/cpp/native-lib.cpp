...
Point intersectionPoint(Vec<float, 2> line1, Vec<float, 2> line2) {
    ...
    pt.y = num_y / dem_y;
    pt.x = (rho1 - pt.y * sin(theta1)) / dem_x;
...
extern "C"
JNIEXPORT void JNICALL Java_com_example_opencv_1test0_EdgeDetection_scanDoc(
        JNIEnv *env,
        jobject /* this */,
        jlong image) {

    cv::Mat& frame = *(cv::Mat *)image, gray, edges;
...
//    frame = Mat(frame, roi);
    ...
    cv::Canny(gray, edges, 20, 30);

    vector<cv::Vec2f> lines, new_lines, hor_lines, ver_lines;

    cv::HoughLines(edges, lines, 1, CV_PI/180, 90);
    ...
    try {
        ...
        if (hor_lines.size() != 0 and ver_lines.size() != 0) {
